...
class WaterProgressComplicationService : ComplicationDataSourceService() {

    override fun getPreviewData(type: ComplicationType): ComplicationData? {
        return createComplicationData(type, 1500, 2000)
    }
    ...
    private fun createComplicationData(
        ...
        val textContent = when {
            ...
            else -> PlainComplicationText.Builder("$totalIntake").build()
        }
        val titleContent = if (isInvisible) null else PlainComplicationText.Builder("ml").build()
        val descContent = PlainComplicationText.Builder("Water progress $totalIntake of $goal ml").build()
        ...
        return when (type) {
            ...
            ComplicationType.LONG_TEXT -> {
                val builder = LongTextComplicationData.Builder(
                    text = if (isInvisible) PlainComplicationText.Builder(" ").build() else PlainComplicationText.Builder("$totalIntake / $goal ml").build(),
                    contentDescription = descContent
                )
                if (!isInvisible) {
