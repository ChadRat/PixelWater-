...
class WaterViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _devQuickAddNotificationEnabled = MutableStateFlow(prefs.getBoolean("dev_quick_add_notification_enabled", false))
    val devQuickAddNotificationEnabled: StateFlow<Boolean> = _devQuickAddNotificationEnabled.asStateFlow()
    ...
    private val _devHapticTestGapMs = MutableStateFlow(prefs.getFloat("dev_haptic_test_gap_ms", 120f))
    val devHapticTestGapMs = _devHapticTestGapMs.asStateFlow()

    fun updateDevHapticTestGapMs(gapMs: Float) {
        _devHapticTestGapMs.value = gapMs
        prefs.edit().putFloat("dev_haptic_test_gap_ms", gapMs).apply()
    }

    fun triggerCustomEnhancedHaptic(type: CustomHapticType, strengthMultiplier: Float = 1.0f) {
        val baseStrength = enhancedHapticStrength.value.value
        val s = (baseStrength * strengthMultiplier).coerceIn(0.01f, 1.0f)
        HapticUtil.performTailoredHaptic(getApplication(), s, type)
    }

    fun triggerDevUnlatchHaptic() {
        HapticManager.triggerUnlatchHaptic(getApplication(), null, _vibrationStrength.value)
    }

    fun triggerDevTensionHaptic() {
        HapticManager.triggerTensionHaptic(getApplication(), null, _vibrationStrength.value)
    }
    ...
    fun updateDevQuickAddNotificationEnabled(enabled: Boolean) {
        _devQuickAddNotificationEnabled.value = enabled
        prefs.edit().putBoolean("dev_quick_add_notification_enabled", enabled).apply()
        if (enabled) {
            com.pixelwater.app.notifications.NotificationHelper.showDevQuickAddNotification(getApplication())
        } else {
            com.pixelwater.app.notifications.NotificationHelper.cancelDevQuickAddNotification(getApplication())
        }
    }
    ...
    private val _deviceBlueprint = MutableStateFlow<com.pixelwater.app.data.DeviceBlueprint?>(null)
    val deviceBlueprint: StateFlow<com.pixelwater.app.data.DeviceBlueprint?> = _deviceBlueprint.asStateFlow()
    ...
    fun generateDeviceBlueprint() {
        if (_isGeneratingBlueprint.value) return
        val currentModel = getActiveModel()
        val currentBrand = getActiveBrand()
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            ...
            try {
                ...
                val prompt = """
                    You are an expert mobile hardware analyst and illustrator. The user is using an Android device: $currentBrand $currentModel.
                    Provide a JSON response with two parts:
                    1. specsDescription: A 2-sentence rich summary of the device's hardware specifications (display type, chipset, battery, release year). Add "Specs powered by GSMArena specs database simulation." at the end.
                    2. A 'blueprint' showing the back of the device, represented on a 100x200 grid system (where the phone back is roughly x=0, y=0 to w=100, h=200).
                    Please approximate its exact camera layout.
                    
                    Return ONLY valid JSON (no markdown block):
                    {
                      "specsDescription": "...",
                      "backPanelColor": "#222222",
                      "bumpX": 20.0, "bumpY": 10.0, "bumpWidth": 60.0, "bumpHeight": 30.0,
                      "bumpCornerRadius": 5.0,
                      "isHorizontalVisor": false, // true ONLY if it looks like a Google Pixel 6/7/8 visor across the whole width
                      "flashX": 70.0, "flashY": 25.0,
                      "lenses": [
                        { "x": 30.0, "y": 25.0, "radius": 8.0 },
                        { "x": 50.0, "y": 25.0, "radius": 8.0 }
                      ]
                    }
                """.trimIndent()
                ...
                val cleanJson = if (start != -1 && end != -1 && end > start) {
                    rawText.substring(start, end + 1).trim()
                } else {
                    rawText.replace("```json", "").replace("```", "").trim()
                }
                val jsonObject = org.json.JSONObject(cleanJson)

                val lensesArray = jsonObject.optJSONArray("lenses")
                val lensesList = mutableListOf<com.pixelwater.app.data.DeviceBlueprint.Lens>()
                if (lensesArray != null) {
                    for (i in 0 until lensesArray.length()) {
                        val lensObj = lensesArray.getJSONObject(i)
                        lensesList.add(
                            com.pixelwater.app.data.DeviceBlueprint.Lens(
                                x = lensObj.optDouble("x").toFloat(),
                                y = lensObj.optDouble("y").toFloat(),
                                radius = lensObj.optDouble("radius").toFloat()
                            )
                        )
                    }
                }

                val blueprint = com.pixelwater.app.data.DeviceBlueprint(
                    specsDescription = jsonObject.optString("specsDescription", "Specs powered by GSMArena specs database simulation."),
                    backPanelColor = jsonObject.optString("backPanelColor", "#222222"),
                    bumpX = jsonObject.optDouble("bumpX").toFloat(),
                    bumpY = jsonObject.optDouble("bumpY").toFloat(),
                    bumpWidth = jsonObject.optDouble("bumpWidth").toFloat(),
                    bumpHeight = jsonObject.optDouble("bumpHeight").toFloat(),
                    bumpCornerRadius = jsonObject.optDouble("bumpCornerRadius").toFloat(),
                    flashX = jsonObject.optDouble("flashX").toFloat(),
                    flashY = jsonObject.optDouble("flashY").toFloat(),
                    isHorizontalVisor = jsonObject.optBoolean("isHorizontalVisor", false),
                    lenses = lensesList,
                    detectedBrand = currentBrand,
                    detectedModel = currentModel
                )

                _deviceBlueprint.value = blueprint

            } catch (e: Exception) {
                Log.e("WaterViewModel", "Failed generating Device Blueprint through AI", e)
                _deviceBlueprint.value = getLocalFallbackBlueprint(currentBrand, currentModel)
            } finally {
    ...
    private fun getLocalFallbackBlueprint(brand: String, model: String): com.pixelwater.app.data.DeviceBlueprint {
        val b = brand.lowercase(java.util.Locale.US)
        val m = model.lowercase(java.util.Locale.US)
        
        var specs = ""
        var backColor = "#1D232A"
        var bumpX = 15f
        var bumpY = 15f
        var bumpWidth = 35f
        var bumpHeight = 55f
        var bumpCornerRadius = 10f
        var flashX = 40f
        var flashY = 45f
        var isHorizontalVisor = false
        val lensesList = mutableListOf<com.pixelwater.app.data.DeviceBlueprint.Lens>()
        
        if (b.contains("google") || m.contains("pixel")) {
            isHorizontalVisor = true
            bumpX = 0f
            bumpY = 25f
            bumpWidth = 100f
            bumpHeight = 18f
            ...
            if (m.contains("pro") || m.contains("6 pro") || m.contains("7 pro") || m.contains("8 pro") || m.contains("9 pro")) {
                backColor = "#D5EDDF" // Sorta Sage
                specs = "6.71-inch LTPO AMOLED (1440x3120, 120Hz) display, Google Tensor chipset, 12GB RAM, 5003 mAh battery, released in 2021-2024. Specs powered by GSMArena specs database simulation (with smart offline helper fallback)."
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(18f, 34f, 4.0f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(34f, 34f, 4.0f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(56f, 34f, 5.0f))
            } else {
                backColor = "#3C4043" // Stormy Black
                specs = "6.4-inch AMOLED 90Hz display, Google Tensor chipset, 8GB RAM, 4614 mAh battery, released in 2021-2024. Specs powered by GSMArena specs database simulation (with smart offline helper fallback)."
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(24f, 34f, 4.5f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(45f, 34f, 4.5f))
            }
        }
        else if (b.contains("samsung") || m.contains("galaxy") || m.contains("sm-")) {
            isHorizontalVisor = false
            
            if (m.contains("ultra")) {
                backColor = "#1D232A" // Black
                specs = "6.8-inch Dynamic AMOLED 2X (120Hz, HDR10+) display, Snapdragon 8 Gen chipset, 12GB RAM, 5000 mAh battery with 45W wired charging, released in 2022-2025. Specs powered by GSMArena specs database simulation (with smart offline helper fallback)."
                bumpX = 0f
                bumpY = 0f
                bumpWidth = 0f
                bumpHeight = 0f
                ...
                flashX = 72f
                flashY = 32f
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(25f, 22f, 5.0f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(25f, 42f, 5.0f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(25f, 62f, 5.0f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(60f, 22f, 3.5f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(60f, 42f, 3.5f))
            } else {
                backColor = "#F9F9FB" // Whiteish
                specs = "6.2-inch Dynamic AMOLED 2X 120Hz display, Exynos/Snapdragon chipset, 8GB RAM, 4000 mAh battery, released in 2021-2024. Specs powered by GSMArena specs database simulation (with smart offline helper fallback)."
                bumpX = 14f
                bumpY = 14f
                bumpWidth = 24f
                bumpHeight = 65f
                bumpCornerRadius = 12f
                flashX = 52f
                flashY = 36f
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(26f, 26f, 4.5f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(26f, 46f, 4.5f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(26f, 66f, 4.5f))
            }
        }
        else if (b.contains("apple") || m.contains("iphone") || b.contains("ios")) {
            isHorizontalVisor = false
            backColor = "#E3E4E5" // Natural Titanium
            
            if (m.contains("pro") || m.contains("max")) {
                specs = "6.7-inch Super Retina XDR OLED (120Hz, HDR10) display, Apple A-series Pro chipset, 8GB RAM, 4441 mAh battery with 20W charging, released recently. Specs powered by GSMArena specs database simulation (with smart offline helper fallback)."
                bumpX = 10f
                bumpY = 10f
                bumpWidth = 42f
                bumpHeight = 42f
                bumpCornerRadius = 14f
                flashX = 42f
                flashY = 21f
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(22f, 21f, 5.5f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(31f, 38f, 5.5f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(40f, 21f, 3.5f))
            } else {
                specs = "6.1-inch Super Retina XDR OLED display, Apple A-series chipset, 6GB RAM, 3279 mAh battery, released recently. Specs powered by GSMArena specs database simulation (with smart offline helper fallback)."
                bumpX = 10f
                bumpY = 10f
                bumpWidth = 38f
                bumpHeight = 38f
                ...
                flashX = 38f
                flashY = 20f
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(20f, 20f, 5.5f))
                lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(30f, 30f, 5.5f))
            }
        }
        else {
            isHorizontalVisor = false
            backColor = "#121212" // Matte Black
            specs = "6.5-inch multi-touch display, Octa-core processor, high-capacity battery, active sensors & dynamic cellular antenna connectivity. Specs powered by GSMArena specs database simulation (with smart offline helper fallback)."
            bumpX = 14f
            bumpY = 14f
            bumpWidth = 30f
            bumpHeight = 55f
            ...
            flashX = 29f
            flashY = 52f
            lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(29f, 24f, 4.5f))
            lensesList.add(com.pixelwater.app.data.DeviceBlueprint.Lens(29f, 38f, 4.5f))
        }
        
        return com.pixelwater.app.data.DeviceBlueprint(
            specsDescription = specs,
            backPanelColor = backColor,
            bumpX = bumpX,
            bumpY = bumpY,
            bumpWidth = bumpWidth,
            ...
            flashX = flashX,
            flashY = flashY,
            isHorizontalVisor = isHorizontalVisor,
            lenses = lensesList,
            detectedBrand = brand,
            detectedModel = model
        )
